Чтобы не вводить номер карты вручную каждый раз, лучше всего настроить автоматическую выдачу реквизитов из пула сохраненных карт.
В обновленный файл bot.js добавлена система хранения карт (cards.json) и следующие команды для администратора:
 * /addcard НомерКарты Владелец — добавить новую карту в пул (например: /addcard 8600123456789012 Иван И.).
 * /cards — посмотреть список всех сохраненных карт и их ID.
 * /delcard ID — удалить карту из базы по её номеру в списке.
Теперь при выборе пополнения P2P UZ бот автоматически берет карту из базы (по очереди или случайно), отправляет реквизиты пользователю, а администратору в чат приходит уведомление с информацией о сделке.
Обновленный файл bot.js
...
app.post('/api/billing/invoice', async (req, res) => {
    ...
    try {
        if (currency === 'P2P UZ') {
            ...
            if (cards.length === 0) {
                return res.json({ success: false, error: 'У администратора не добавлены карты для приема P2P UZ. Обратитесь в поддержу.' });
            }

            // Автоматический выбор карты по кругу (ротация)
            const activeCard = cards[cardIndex % cards.length];
            cardIndex = (cardIndex + 1) % cards.length;

            // Отправляем реквизиты пользователю автоматически
            ...
            // Уведомляем администратора в чате
            if (ADMIN_CHAT_ID && ADMIN_CHAT_ID !== 'YOUR_ADMIN_CHAT_ID') {
                await bot.sendMessage(ADMIN_CHAT_ID, 
                    ...
                    `Ожидайте подтверждения оплаты от пользователя.`, 
                    {
                        parse_mode: 'Markdown',
                        reply_markup: {
                            inline_keyboard: [
            ...
            await bot.sendMessage(tgId, 
                ...
                { parse_mode: 'Markdown', reply_markup: { inline_keyboard: [[{ text: '💎 Оплатить в CryptoBot', url: payUrl }]] } }
            );
...
bot.on('message', async (msg) => {
    ...
    // Управление картами через команды админа
    if (text.startsWith('/addcard')) {
        const parts = text.split(' ');
        if (parts.length < 3) {
            await bot.sendMessage(msg.chat.id, '❌ Формат: `/addcard [номер_карты] [владелец]`', { parse_mode: 'Markdown' });
            return;
        }
        ...
        saveCards();
        await bot.sendMessage(msg.chat.id, `✅ Карта \`${number}\` (${holder}) успешно добавлена в пул! Всего карт: ${cards.length}`, { parse_mode: 'Markdown' });
        return;
    }

    if (text.startsWith('/cards')) {
        if (cards.length === 0) {
            await bot.sendMessage(msg.chat.id, '📭 Список карт пуст. Добавьте карту через `/addcard`.', { parse_mode: 'Markdown' });
            return;
        }
    ...
    if (text.startsWith('/delcard')) {
        ...
        saveCards();
        await bot.sendMessage(msg.chat.id, `🗑 Карта \`${removed[0].number}\` удалена из пула.`, { parse_mode: 'Markdown' });
        return;
    }
